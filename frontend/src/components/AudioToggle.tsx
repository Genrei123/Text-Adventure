...
const AudioToggle: React.FC<AudioToggleProps> = ({ 
  ...
  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.volume = isMuted ? 0 : previousVolume.current;
  ...
  const toggleMute = () => {
    ...
    if (audioRef.current) {
      if (!isMuted) {
        previousVolume.current = audioRef.current.volume > 0 ? audioRef.current.volume : previousVolume.current;
      }
    }
  };

  return (
    <StyledWrapper position={position} color={color} size={size} className={className}>
